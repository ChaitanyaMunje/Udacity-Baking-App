...
public class Database extends SQLiteOpenHelper {
    ...
    public void insertItem(WidgetModel model, int widgetId) {
        ...
        for (int i = 0; i < model.ingredients.size(); i++) {
            values2.put(Contract.COLUMN_INGREDIENT, model.ingredients.get(i).getIngredient());
            values2.put(Contract.COLUMN_MEASURE, model.ingredients.get(i).getMeasure());
            values2.put(Contract.COLUMN_QUANTITY, Float.valueOf(model.ingredients.get(i).getQuantity()));
